        ...
        const response = await fetch(API_CONFIG.url + '/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                        {
                            role: "system",
                            content: `料理アシスタントです。料理に関する質問には、自分の知識ではなく必ずsearch_recipes関数を実行してレシピデータベースから情報を取得してください。推測や一般論は避け、データベースの具体的な情報のみで回答します。材料、手順、コツ、アレンジ方法など詳しい作り方を親しみやすく温かい口調で説明します。`
                        },
                    {
                        role: "user",
                        content: message
            ...
            const finalResponse = await fetch(API_CONFIG.url + '/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: "system",
                            content: `料理アシスタントです。関数実行結果のデータベース情報のみを使用して回答してください。自分の知識や推測は使わず、取得したレシピデータのみで説明します。材料、手順、コツ、アレンジ方法など詳しい作り方を親しみやすく温かい口調で詳しく説明してください。`
                        },
                        {
                            role: "user",
                            content: message
