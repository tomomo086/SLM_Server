...
def extract_1_20_pdf_content(pdf_path, max_pages=10):
    """1-20.pdfから内容を抽出する"""
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            print(f"1-20.pdfの総ページ数: {len(pdf.pages)}")
            ...
            for page_num in range(max_pages):
                ...
                if page_text and page_text.strip():
                    ...
                    try:
                        ...
                        images_dir = "data/pdf_images_1_20"
...
def create_1_20_knowledge_entries(text_content):
    """1-20.pdfの内容から知識エントリを作成する"""
    if not text_content:
        return []
    
    # 1-20.pdfの内容を基にした知識エントリ（易の本の前半部分を想定）
    knowledge_entries = [
        {
...
def main():
    pdf_path = "data/易の元ファイル/1-20.pdf"
    
    if not os.path.exists(pdf_path):
        print(f"PDFファイルが見つかりません: {pdf_path}")
        return
    
    print("1-20.pdfから内容を抽出中...")
    print("注意: 最初の10ページを処理します")
    
    # PDFからテキストを抽出
    text_content = extract_1_20_pdf_content(pdf_path, max_pages=10)
    ...
    # 知識エントリを作成
    print("\n1-20.pdfの内容を基に知識エントリを作成中...")
    knowledge_entries = create_1_20_knowledge_entries(text_content)
